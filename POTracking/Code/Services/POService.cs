        ...
        internal bool IsPOAccessible(int POId, int UserId, int OrgId)
        {
            if (_Session.IsAsiaVendor) // Special Case for Asia Vendor
                return (dbc.POHeaders.Where(c =>
                    (c.VendorID == Config.VendorIDDeestone || c.VendorID == Config.VendorIDDeestoneLtd || c.VendorID == Config.VendorIDSvizz || c.VendorID == Config.VendorIDSiamtruck || c.VendorID == Config.VendorIDDeestoneCorpLtd)
                    && c.ID == POId).Count() > 0);
